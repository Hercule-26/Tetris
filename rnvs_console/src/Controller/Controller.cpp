...
#include "../Model/Direction.h" // rnvs : ajout
...
Controller::Controller(TetrisInterface & game,
                       View & view) : game(game), view(view)
{
    game.Register(&view);
}
...
void Controller::start()
{
    ...
    */
    ...
    while (!game.is_game_over())
        // while (true)    // rnvs : pour test
    {
        cout << "Entrez une commande \n(s: down, q: left, d: right | f: drop | e: rotate rigth | a: rotate left)"
             << endl;
        getline(cin, input);
        try
        {
