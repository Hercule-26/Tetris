...
#include "Direction.h"  // rnvs : ajoutµ
...
Game::Game(int lvl, int row, int col) : score(0), bag(), board({row, col}),
     level(lvl), counteCompletedLine(0)
{
...
void Game::generateRandomBoard()
{
    ...
    unsigned maximumShapes = (board.get_board_col() +
                              board.get_board_row()) / 4;
...
void Game::rotate_shape(Rotation dir)
{
    ...
    if (!rotation_is_possible())
    {
        switch (dir)
        {
            case Rotation::ROTATE_LEFT : curr_shape.rotate(
                        Rotation::ROTATE_RIGHT);
                break;
            case Rotation::ROTATE_RIGHT : curr_shape.rotate(
...
bool Game::is_movable(Direction2D dir)
{
    ...
    for (unsigned i = 0; i < pos.size(); ++i)
    {
        ...
        tmp_pos = tmp_pos + dir;
        if (board.is_out_of_board(tmp_pos) ||
                !board.is_cell_empty(tmp_pos)) return false;
    }
    return true;
}
